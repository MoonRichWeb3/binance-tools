...
| 状态 | 功能 | 说明 |
| --- | --- | --- |
| [✓] 【完成】 | Alpha 知识库 | 已创建 `docs/知识库/币安api/alpha/` |
| [✓] 【完成】 | Alpha REST 文档 | 已整理 Token 列表、交易对信息、聚合交易、K 线、24hr ticker、完整深度 |
| [✓] 【完成】 | Alpha WebSocket 文档 | 已整理订阅协议和官方 stream 清单 |
| [✓] 【完成】 | Alpha REST 客户端 | `AlphaClient` 已实现 6 个公开市场数据 REST 接口 |
| [✓] 【完成】 | Alpha WebSocket 辅助 | 已实现订阅/取消订阅/查询订阅消息和 stream 名称构造 |
| [✓] 【完成】 | 单元测试 | 已覆盖 URL 构造、K 线数组解析和 WebSocket stream 名称 |
| [✓] 【完成】 | Alpha 菜单入口 | 顶部菜单栏新增 `Alpha`，包含 `Token列表`、`交易对信息`、`热力图` 和 `日均线信号` |
| [✓] 【完成】 | Alpha Token 页面 | 可查询并展示 Alpha Token 列表，支持 `积分+`（仅 `mulPoint` 为 2 或 4）、`证券代币`、链分类和关键字搜索，表头使用中文，代币名称后展示 `mulPoint` 倍率，并可点击 K 线图标进入 120 日 Alpha K 线图 |
| [✓] 【完成】 | Alpha 热力图 | 新增 Alpha 热力图页面，复用现货热力图排版；市值模式使用 `marketCap`，成交额模式使用 `volume24h`，支持链筛选、搜索和点击进入 Alpha K 线图 |
| [✓] 【完成】 | Alpha 交易对页面 | 支持搜索 Symbol/Base/Quote/状态、按计价资产切换，并按当前筛选结果统计上涨、下跌、持平数量 |
| [✓] 【完成】 | Alpha SQLite 缓存 | 新增 `alpha_tokens`、`alpha_assets`、`alpha_symbols`，页面优先读库，手动刷新才请求 Binance |
| [✓] 【完成】 | Alpha 日均线信号 | 新增 `alpha_klines` 缓存表和 Alpha 日均线页面，查询逻辑与现货一致：先读库、缺失时补日 K |
| [✓] 【完成】 | Alpha K 线图 | 复用现货 K 线图交互能力，Alpha 数据源读取 `alpha_klines`，Token 列表点击时按单个 Alpha USDT 交易对补齐 120 日日线；若本地缓存最新日线不是当天，会自动重新拉取补齐 |
| [✓] 【完成】 | 证券代币分类 | 同时满足 `stockState = true` 且符号/名称/CEX 名称带 `on` 后缀时归类为证券代币 |
...
| 状态 | 接口 | 本地函数 |
| --- | --- | --- |
| [✓] 【完成】 | Token 列表 | `AlphaClient::token_list` |
| [✓] 【完成】 | 交易对信息 | `AlphaClient::exchange_info` |
| [✓] 【完成】 | 聚合交易 | `AlphaClient::aggregate_trades` |
| [✓] 【完成】 | K 线 | `AlphaClient::klines` |
| [✓] 【完成】 | 24hr 价格变动 | `AlphaClient::ticker_24hr` |
| [✓] 【完成】 | 完整深度 | `AlphaClient::full_depth` |
